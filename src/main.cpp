...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        mainMenu();
    } else if (argc >= 3) {  // If lab name is given then generate default
...
void buildLab(Lab lab) {
    ...
    if (lab.destExists()) {
        ...
        if (delOpt.compare("d") || delOpt.compare("delete")) {
            ...
            if (confirmDel == 'y') {
                ...
                std::cout << "Deleted directory " << lab.getFullPath()
                          << " folder containing " << n << " files.\n\n";
            }
    ...
    if (defOpt == 'n') {
        ...
        std::cout << "Enter path to lab parent directory ('.' for current "
                     "directory): ";
        std::cin >> pDir;
